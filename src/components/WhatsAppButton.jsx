...
const BASE = `position:fixed; z-index:60;
  right:calc(var(--safe-r) + clamp(16px, 2.2vw, 28px));
  bottom:calc(var(--safe-b) + clamp(16px, 2.2vw, 28px));
  width:clamp(52px, 4vw, 58px); height:clamp(52px, 4vw, 58px);
  display:flex; align-items:center; justify-content:center;
  border-radius:999px; background:#25D366; color:#FFFFFF;
  box-shadow:0 12px 30px -10px rgba(28,25,23,0.45), 0 2px 6px -2px rgba(28,25,23,0.25);
  transition:transform .22s cubic-bezier(.2,.8,.2,1), box-shadow .22s ease, background .22s ease`;

const HOVER = `background:#1FBE59;
  transform:translateY(-2px) scale(1.04);
  box-shadow:0 18px 40px -12px rgba(28,25,23,0.5), 0 2px 6px -2px rgba(28,25,23,0.25)`;
...
export function WhatsAppButton() {
  ...
  return (
    <Hover
      ...
      style={hidden ? `${BASE}; opacity:0; transform:translateY(8px) scale(0.9); pointer-events:none` : BASE}
      hoverStyle={HOVER}
    >
