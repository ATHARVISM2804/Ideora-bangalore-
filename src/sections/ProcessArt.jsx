  ...
  return (
    <div style={s('position:absolute; inset:0; display:flex; align-items:center; justify-content:center')}>
      <div style={s('position:relative; width:190px; height:190px')}>
        <div style={s('position:absolute; inset:0; border-radius:50%; border:1px solid var(--rule-strong)')} />
        <div style={s('position:absolute; inset:26px; border-radius:50%; border:1px solid var(--rule)')} />
        <div style={s('position:absolute; inset:52px; border-radius:50%; border:1px solid var(--rule)')} />
        <div style={s('position:absolute; inset:78px; border-radius:50%; border:1px solid var(--rule)')} />
        ...
        <div style={s('position:absolute; left:70%; top:28%; width:6px; height:6px; border-radius:50%; background:var(--accent-deep); animation:om-ping 2.2s ease-in-out infinite')} />
        <div style={s('position:absolute; left:26%; top:62%; width:5px; height:5px; border-radius:50%; background:var(--ink-muted); animation:om-ping 2.8s ease-in-out .4s infinite')} />
        <div style={s('position:absolute; left:58%; top:76%; width:5px; height:5px; border-radius:50%; background:var(--ink-muted); animation:om-ping 3.1s ease-in-out .9s infinite')} />
      </div>
    </div>
  );
}
  ...
  return (
    <div style={s('position:absolute; inset:0; padding:26px')}>
      <svg viewBox="0 0 300 200" style={s('width:100%; height:100%; overflow:visible')}>
        ...
        <path d="M20 110 H96 a12 12 0 0 1 12 12 V158 H240" fill="none" stroke="var(--ink)" strokeWidth="1.5" opacity="0.35" strokeDasharray="520" style={s('animation:om-draw 4.5s ease-in-out .7s infinite')} />
        <rect x="150" y="96" width="110" height="20" rx="6" fill="rgba(244,96,30,0.12)" />
        <rect x="150" y="128" width="82" height="8" rx="4" fill="rgba(28,25,23,0.16)" />
        <rect x="20" y="140" width="56" height="8" rx="4" fill="var(--rule-strong)" />
        <line x1="12" y1="10" x2="12" y2="180" stroke="rgba(28,25,23,0.2)" strokeWidth="1" />
      </svg>
    </div>
  );
}
...
function Code() {
  return (
    <div style={s('position:absolute; inset:22px; border-radius:12px; overflow:hidden; border:1px solid var(--dark-rule); background:var(--dark); box-shadow:0 18px 40px -24px rgba(28,25,23,0.8)')}>
      <div style={s('display:flex; align-items:center; gap:6px; padding:10px 12px; border-bottom:1px solid var(--dark-rule); background:#191512')}>
        <span style={s('width:9px; height:9px; border-radius:50%; background:#F4601E')} />
        <span style={s('width:9px; height:9px; border-radius:50%; background:var(--ink-muted)')} />
        <span style={s('width:9px; height:9px; border-radius:50%; background:var(--dark-rule)')} />
        <span style={s(`margin-left:10px; ${MONO}; font-size:10px; color:#8A8177`)}>agent.run.js</span>
      </div>
      <div style={s(`padding:14px; display:flex; flex-direction:column; gap:8px; ${MONO}; font-size:11px; color:#E8E2DA`)}>
        ...
        <div style={s('margin-top:6px; display:flex; align-items:center; gap:7px; font-size:10px; color:#8A8177')}>
  ...
  return (
    <div style={s('position:absolute; inset:0; padding:24px')}>
      <svg viewBox="0 0 300 200" style={s('width:100%; height:100%')}>
        ...
        <path d="M212 100 H262" fill="none" stroke="var(--ink)" strokeWidth="1.6" opacity=".4" strokeDasharray="6 6" style={s('animation:om-dash 1.6s linear infinite')} />
        <g fill="#FFFFFF" stroke="var(--rule-strong)">
...
const DASH_BARS = [
  ['44%', 'var(--rule-strong)', ''],
  ['62%', 'var(--rule-strong)', ' .2s'],
  ['52%', 'var(--rule-strong)', ' .4s'],
  ['78%', 'var(--rule-strong)', ' .6s'],
  ['100%', '#F4601E', ' .8s'],
  ['70%', 'var(--rule-strong)', ' 1s'],
  ['56%', 'var(--rule-strong)', ' 1.2s'],
];

function Dash() {
  return (
    <div style={s('position:absolute; inset:22px; border-radius:14px; border:1px solid var(--rule); background:#FFFFFF; padding:18px; display:flex; flex-direction:column')}>
      <div style={s(`display:flex; justify-content:space-between; ${MONO}; font-size:10px; color:var(--ink-muted)`)}>
        <span>operations digest</span><span style={s('color:var(--accent-deep)')}>Thu 18:00</span>
      </div>
      ...
      <div style={s(`margin-top:auto; padding-top:14px; border-top:1px solid var(--rule); display:flex; justify-content:space-between; ${MONO}; font-size:10px; color:var(--ink-muted)`)}>
