    ...
    <section id="top" style={s('padding:clamp(36px, 5vw, 64px) 0 0')}>
      <div style={s('position:relative; max-width:1400px; margin:0 auto; padding:0 clamp(20px, 5vw, 40px)')}>
        <div style={s('position:relative; z-index:1; display:flex; align-items:center; gap:12px; flex-wrap:wrap')} data-anim="hero-1">
          <span style={s('display:inline-flex; align-items:center; gap:8px; padding:6px 13px 6px 10px; border-radius:99px; border:1px solid var(--rule); background:var(--raised); font-size:13px; color:var(--ink-muted)')}>
            <span style={s('width:6px; height:6px; border-radius:50%; background:var(--accent)')} />
            Four systems running in production
          </span>
          <span style={s('font-size:13px; color:var(--ink-faint)')}>Automotive, real estate and healthcare operations</span>
        </div>
        ...
        <div className="om-g12" style={s('margin-top:clamp(28px, 4vw, 36px); display:grid; grid-template-columns:repeat(12,1fr); gap:20px; align-items:start')} data-anim="hero-2">
          ...
          <div style={s('grid-column:1 / span 6')}>
            <p style={s('margin:0; max-width:48ch; font-size:18px; line-height:1.6; color:var(--ink-muted)')}>Ideora Labs builds agentic AI that works inside your existing operations. It books, checks, approves, updates, and reports without anyone chasing it.</p>
            <div style={s(`margin-top:30px; display:flex; align-items:center; gap:${phone ? '10px' : '14px'}; flex-wrap:wrap`)}>
              <Hover
                as="a"
                href="#services"
                style="display:inline-flex; align-items:center; padding:13px 22px; border-radius:8px; border:1px solid var(--rule-strong); background:var(--raised); color:var(--ink); font-size:15px; font-weight:500; transition:background .25s, border-color .25s"
              ...
              <Hover
                ...
                style="display:inline-flex; align-items:center; padding:14px 26px; border-radius:8px; background:var(--accent); color:#FFFFFF; font-size:15px; font-weight:500; transition:transform .18s ease-out, background .25s"
      ...
      <div style={s('margin-top:clamp(44px, 7vw, 76px); border-top:1px solid var(--rule); border-bottom:1px solid var(--rule); background:var(--bg-sunken)')}>
        <div style={s('max-width:1400px; margin:0 auto; padding:22px clamp(20px, 5vw, 40px); display:flex; align-items:baseline; gap:clamp(16px, 3vw, 40px); flex-wrap:wrap')}>
          <span style={s('font-size:13px; color:var(--ink-faint); flex:none')}>Practices</span>
          {PRACTICES.map((n) => (
