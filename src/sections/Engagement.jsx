            ...
            {STEPS.map((st) => (
              <div key={st.n} data-anim="step" style={s('padding-right:28px')}>
                ...
                <div style={s('margin-top:32px; display:flex; align-items:center; gap:12px')}>
                  <span style={s('width:28px; height:28px; border-radius:9px; border:1px solid rgba(26,29,35,0.1); background:#FFFFFF; display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:500; color:#5A616D')}>{st.n}</span>
                  <h3 style={s('margin:0; font-family:Archivo, sans-serif; font-stretch:125%; font-weight:600; font-size:18px; line-height:1.15; letter-spacing:-0.02em')}>{st.title}</h3>
                </div>
                <p style={s('margin:14px 0 0; font-size:15px; color:#5A616D')}>{st.body}</p>
              </div>
            ))}
          </div>
        </div>
