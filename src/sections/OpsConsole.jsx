...
export function OpsConsole({ consoleRef }) {
  ...
  return (
    <div ref={consoleRef} onMouseMove={spot} style={s('position:relative; max-width:1400px; margin:0 auto; overflow:hidden; border-radius:20px; border:1px solid #2E333C; background:#16191F; color:#F1F3F6; box-shadow:0 60px 120px -60px rgba(26,29,35,0.9)')} data-anim="console">
      ...
      {/* Title bar */}
      <div style={s(`position:relative; display:flex; align-items:center; justify-content:space-between; gap:24px; padding:13px 20px; border-bottom:${LINE}; background:#12151A`)}>
        ...
        <div role="tablist" aria-label="Industry" style={s(`display:flex; align-items:center; gap:2px; padding:3px; border-radius:12px; border:${LINE}; background:#1A1D23`)}>
          {DASHBOARDS.map((item, i) => {
            const on = i === active;
            return (
              <button key={item.id} type="button" role="tab" aria-selected={on}
                onClick={() => { setActive(i); setAutoplay(false); }}
                style={s(`padding:6px 13px; border:0; border-radius:9px; cursor:pointer; font-family:inherit; font-size:12px; font-weight:500; background:${on ? 'rgba(244,96,30,0.16)' : 'transparent'}; color:${on ? '#FF8A50' : '#8A929E'}; transition:background .3s, color .3s`)}
              >{item.name}</button>
            );
          })}
        </div>

        <div style={s(`display:flex; gap:18px; ${MONO}; font-size:11px; color:#8A929E; flex:none`)}>
          <span>ap-south-1</span><span>uptime 99.98%</span><span>{d.clock}</span>
        </div>
      </div>

      <div ref={bodyRef} style={s('position:relative; display:grid; grid-template-columns:214px 1fr 306px; min-height:600px')}>

        {/* ── Sidebar ────────────────────────────────────────────── */}
        <div style={s(`border-right:${LINE}; display:flex; flex-direction:column; background:#14171C`)}>
        ...
        <div style={s('display:flex; flex-direction:column; min-width:0; background:#181B21')}>
          {/* Toolbar */}
          <div style={s(`display:flex; align-items:center; gap:10px; padding:12px 18px; border-bottom:${LINE}`)}>
            ...
            <span style={s('display:inline-flex; align-items:center; gap:7px; flex:none; padding:8px 14px; border-radius:10px; background:#F4601E; color:#1A1D23; font-size:12px; font-weight:500')}>
              <Icon name="plus" size={13} />{d.action}
            </span>
          </div>

          {/* KPI cards */}
          <div style={s('display:grid; grid-template-columns:repeat(4,1fr); gap:10px; padding:14px 18px')}>
            {d.kpis.map((k) => {
              ...
              return (
                <div key={k.label} data-anim="kpi" style={s(`padding:13px 14px 10px; border-radius:13px; border:${LINE}; background:${PANEL}`)}>
                  ...
                  <div style={s('margin-top:8px; display:flex; align-items:baseline; gap:5px')}>
                    <Count value={k.value} style={s(`font-family:Archivo, sans-serif; font-stretch:125%; font-weight:500; font-size:26px; line-height:1; letter-spacing:-0.03em; color:${accent}`)} />
          ...
          {/* Trace table */}
          <div style={s('padding:2px 18px 0; display:flex; align-items:center; justify-content:space-between')}>
          ...
          <div style={s('margin:10px 18px 0; flex:1; display:flex; flex-direction:column; border-radius:13px; border:' + LINE + '; background:' + PANEL + '; overflow:hidden')}>
            <div style={s(`display:grid; grid-template-columns:84px 92px 1fr 84px; gap:12px; padding:9px 14px; border-bottom:${LINE}; background:#1A1D23; ${MONO}; font-size:10px; letter-spacing:0.06em; color:#8A929E`)}>
              <span>TIME</span><span>ACTION</span><span>DETAIL</span><span style={s('text-align:right')}>STATUS</span>
            </div>
            {d.trace.map(([time, op, msg, st], i) => (
              <div key={i} data-anim="trace-row" style={s(`display:grid; grid-template-columns:84px 92px 1fr 84px; gap:12px; align-items:center; padding:8px 14px; ${i % 2 ? 'background:rgba(255,255,255,0.014)' : ''}; ${MONO}; font-size:12.5px; color:#E4E8EE`)}>
                <span style={s('color:#8A929E')}>{time}</span>
                <span style={s('color:#A7AEBA')}>{op}</span>
            ...
            <div data-anim="trace-row" style={s(`display:grid; grid-template-columns:84px 92px 1fr 84px; gap:12px; align-items:center; padding:8px 14px; border-top:${LINE}; background:rgba(244,96,30,0.05); ${MONO}; font-size:12.5px; color:#FF8A50`)}>
          ...
          <div style={s(`margin-top:auto; padding:13px 18px; display:flex; justify-content:space-between; ${MONO}; font-size:11px; color:#8A929E`)}>
            {d.footer.map((f) => <span key={f}>{f}</span>)}
          </div>
        </div>

        {/* ── Right column ───────────────────────────────────────── */}
        <div style={s(`border-left:${LINE}; display:flex; flex-direction:column; background:#14171C`)}>
          <div style={s(`padding:18px 18px; border-bottom:${LINE}; display:flex; align-items:center; gap:16px`)}>
            <Ring value={d.sla} />
