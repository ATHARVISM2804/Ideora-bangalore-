...
const LINE = '1px solid var(--dark-rule)';
const PANEL = 'var(--dark-raised)';

const STATUS = {
  live:       { dot: '#F4601E', text: '#FF8A50', border: 'rgba(244,96,30,0.4)',    bg: 'rgba(244,96,30,0.12)' },
  'in build': { dot: '#8A8177', text: '#C4BCB2', border: 'rgba(196,188,178,0.28)', bg: 'rgba(196,188,178,0.08)' },
  pilot:      { dot: '#8A8177', text: '#C4BCB2', border: 'rgba(196,188,178,0.28)', bg: 'rgba(196,188,178,0.08)' },
};
...
export function OpsConsole({ consoleRef }) {
  ...
  return (
    <div ref={consoleRef} onMouseMove={spot} style={s('position:relative; max-width:var(--wide); margin:0 auto; overflow:hidden; border-radius:20px; border:1px solid var(--dark-rule); background:#191512; color:#F2EDE7; box-shadow:0 40px 90px -50px rgba(31,27,24,0.55)')} data-anim="console">
      ...
      {/* Title bar */}
      <div style={s(`position:relative; display:flex; align-items:center; justify-content:space-between; gap:${compact ? '10px' : '24px'}; ${compact ? 'flex-wrap:wrap' : ''}; padding:13px clamp(14px, 3vw, 20px); border-bottom:${LINE}; background:#17130F`)}>
        ...
        <div role="tablist" aria-label="Industry" className={compact ? 'om-rail' : undefined} style={s(`display:flex; align-items:center; gap:2px; padding:3px; border-radius:12px; border:${LINE}; background:var(--dark); ${compact ? 'overflow-x:auto; max-width:100%' : ''}`)}>
          {DASHBOARDS.map((item, i) => {
            const on = i === active;
            return (
              <button key={item.id} type="button" role="tab" aria-selected={on}
                onClick={() => { setActive(i); setAutoplay(false); }}
                style={s(`flex:none; padding:6px 13px; border:0; border-radius:9px; cursor:pointer; font-family:inherit; font-size:12px; font-weight:500; white-space:nowrap; background:${on ? 'rgba(255,255,255,0.07)' : 'transparent'}; color:${on ? '#F2EDE7' : '#8A8177'}; transition:background .3s, color .3s`)}
              >{item.name}</button>
            );
          })}
        </div>

        <div style={s(`display:${compact ? 'none' : 'flex'}; gap:18px; ${MONO}; font-size:11px; color:#8A8177; flex:none`)}>
      ...
      <div ref={bodyRef} style={s(`position:relative; display:grid; grid-template-columns:${compact ? '1fr' : '214px 1fr 306px'}; ${compact ? '' : 'min-height:600px'}`)}>
        ...
        {/* ── Sidebar ────────────────────────────────────────────── */}
        <div style={s(`border-right:${LINE}; display:${compact ? 'none' : 'flex'}; flex-direction:column; background:#1C1815`)}>
          <div style={s(`display:flex; align-items:center; gap:10px; padding:16px 16px; border-bottom:${LINE}`)}>
            <span style={s('width:28px; height:28px; flex:none; border-radius:9px; background:linear-gradient(140deg,#F4601E,var(--accent-deep)); display:flex; align-items:center; justify-content:center; font-family:var(--serif); font-weight:600; font-size:12px; color:var(--dark)')}>{d.name[0]}</span>
            <span style={s('min-width:0')}>
              ...
              <span style={s(`display:block; ${MONO}; font-size:10px; color:#8A8177`)}>{d.name.toLowerCase()}</span>
            </span>
            <span style={s('margin-left:auto; color:#6E6862')}><Icon name="chevron" size={14} /></span>
          </div>

          <div style={s('padding:12px 12px; display:flex; flex-direction:column; gap:3px')}>
            {d.rail.map((r) => (
              <div key={r.label} style={s(`display:flex; align-items:center; gap:10px; padding:9px 11px; border-radius:9px; background:${r.active ? 'rgba(255,255,255,0.06)' : 'transparent'}; color:${r.active ? '#F2EDE7' : '#A79E93'}; font-size:13px; font-weight:500`)}>
                <Icon name={r.icon} size={15} />
                <span>{r.label}</span>
                {r.count && (
                  <span style={s(`margin-left:auto; padding:1px 7px; border-radius:99px; background:${r.active ? 'rgba(255,255,255,0.09)' : 'rgba(196,188,178,0.08)'}; ${MONO}; font-size:10px; color:${r.active ? '#E8E2DA' : '#8A8177'}`)}>{r.count}</span>
                )}
              </div>
            ))}
          </div>
          ...
          <div style={s('margin-top:auto; padding:12px')}>
            <div style={s(`padding:12px 13px; border-radius:12px; border:${LINE}; background:${PANEL}`)}>
              <div style={s(`${MONO}; font-size:10px; color:#8A8177`)}>agents online</div>
              <div style={s('margin-top:6px; display:flex; align-items:baseline; gap:5px')}>
                ...
                <span style={s(`${MONO}; font-size:10px; color:#8A8177`)}>/ {d.online[1]}</span>
              </div>
            </div>
            <div style={s('margin-top:10px; display:flex; align-items:center; gap:10px; padding:8px 4px')}>
              <span style={s(`width:28px; height:28px; flex:none; border-radius:50%; border:${LINE}; background:${PANEL}; display:flex; align-items:center; justify-content:center; ${MONO}; font-size:10px; color:#C4BCB2`)}>{initials}</span>
              <span style={s('min-width:0')}>
                <span style={s('display:block; font-size:12px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis')}>{name}</span>
                <span style={s('display:block; font-size:10.5px; color:#8A8177')}>{role}</span>
              </span>
            </div>
          </div>
        </div>
        ...
        {/* ── Main ───────────────────────────────────────────────── */}
        <div style={s('display:flex; flex-direction:column; min-width:0; background:#221E1A')}>
          {/* Toolbar */}
          <div style={s(`display:flex; align-items:center; gap:10px; padding:12px clamp(14px, 3vw, 18px); border-bottom:${LINE}`)}>
            <div style={s(`display:flex; align-items:center; gap:9px; flex:1; min-width:0; padding:8px 12px; border-radius:10px; border:${LINE}; background:#17130F; color:#6E6862`)}>
              <Icon name="search" size={14} />
              <span style={s('font-size:12.5px; color:#6E6862; white-space:nowrap; overflow:hidden; text-overflow:ellipsis')}>{d.search}</span>
            </div>
            {!compact && d.filters.map((f) => (
              <span key={f} style={s(`display:inline-flex; align-items:center; gap:7px; flex:none; padding:7px 12px; border-radius:10px; border:${LINE}; background:var(--dark); font-size:12px; color:#A79E93`)}>
                {f}<span style={s('color:#6E6862')}><Icon name="chevron" size={12} /></span>
              </span>
            ))}
            <span style={s('display:inline-flex; align-items:center; gap:7px; flex:none; padding:8px clamp(10px, 2.5vw, 14px); border-radius:10px; background:#F4601E; color:var(--dark); font-size:12px; font-weight:500')}>
          ...
          <div className="om-gkpi" style={s('display:grid; grid-template-columns:repeat(4,1fr); gap:10px; padding:14px clamp(14px, 3vw, 18px)')}>
            {d.kpis.map((k) => {
              const accent = k.accent ? '#FF8A50' : '#F2EDE7';
              ...
              return (
                <div key={k.label} data-anim="kpi" style={s(`padding:13px 14px 10px; border-radius:13px; border:${LINE}; background:${PANEL}`)}>
                  <div style={s(`${MONO}; font-size:10px; color:#8A8177; white-space:nowrap; overflow:hidden; text-overflow:ellipsis`)}>{k.label}</div>
                  <div style={s('margin-top:8px; display:flex; align-items:baseline; gap:5px')}>
                    ...
                    <span style={s(`${MONO}; font-size:10px; color:#8A8177`)}>{k.unit}</span>
                    <span style={s(`margin-left:auto; ${MONO}; font-size:10px; color:${k.trend === 'flat' ? '#6E6862' : trendUp ? '#FF8A50' : '#8A8177'}`)}>
                      {k.trend === 'flat' ? '–' : trendUp ? '↑' : '↓'} {k.delta}
                    </span>
                  </div>
                  <div style={s('margin-top:9px')}>
                    <Spark values={k.spark} color={k.accent ? '#F4601E' : '#6E6862'} h={26} />
                  </div>
                </div>
              );
            })}
          ...
          <div style={s('padding:2px clamp(14px, 3vw, 18px) 0; display:flex; align-items:center; justify-content:space-between')}>
            <span style={s(`${MONO}; font-size:11px; color:#8A8177`)}>agent trace · {d.agent} · run {d.run}</span>
            <span style={s(`display:inline-flex; align-items:center; gap:6px; ${MONO}; font-size:11px; color:#8A8177`)}>
          ...
          <div style={s('margin:10px clamp(14px, 3vw, 18px) 0; flex:1; display:flex; flex-direction:column; border-radius:13px; border:' + LINE + '; background:' + PANEL + '; overflow:hidden')}>
            <div style={s(`display:grid; grid-template-columns:${phone ? '66px 1fr 58px' : '84px 92px 1fr 84px'}; gap:12px; padding:9px clamp(10px, 2.5vw, 14px); border-bottom:${LINE}; background:var(--dark); ${MONO}; font-size:10px; letter-spacing:0.06em; color:#8A8177`)}>
            ...
            {d.trace.map(([time, op, msg, st], i) => (
              <div key={i} data-anim="trace-row" style={s(`display:grid; grid-template-columns:${phone ? '66px 1fr 58px' : '84px 92px 1fr 84px'}; gap:12px; align-items:center; padding:8px clamp(10px, 2.5vw, 14px); ${i % 2 ? 'background:rgba(255,255,255,0.014)' : ''}; ${MONO}; font-size:${phone ? '11.5px' : '12.5px'}; color:#E8E2DA`)}>
                <span style={s('color:#8A8177')}>{time}</span>
                {!phone && <span style={s('color:#A79E93')}>{op}</span>}
            ...
            <div data-anim="trace-row" style={s(`display:grid; grid-template-columns:${phone ? '66px 1fr 58px' : '84px 92px 1fr 84px'}; gap:12px; align-items:center; padding:8px clamp(10px, 2.5vw, 14px); border-top:${LINE}; background:rgba(244,96,30,0.06); ${MONO}; font-size:${phone ? '11.5px' : '12.5px'}; color:#FF8A50`)}>
              <span style={s('color:#8A8177')}>{d.done[0]}</span>
          ...
          <div style={s(`margin-top:auto; padding:13px clamp(14px, 3vw, 18px); display:flex; justify-content:space-between; ${MONO}; font-size:11px; color:#8A8177`)}>
            {d.footer.map((f) => <span key={f}>{f}</span>)}
          </div>
        </div>

        {/* ── Right column ───────────────────────────────────────── */}
        <div style={s(`border-${compact ? 'top' : 'left'}:${LINE}; display:flex; flex-direction:column; background:#1C1815`)}>
          <div style={s(`padding:18px clamp(14px, 3vw, 18px); border-bottom:${LINE}; display:flex; align-items:center; gap:16px`)}>
            ...
            <div style={s('min-width:0')}>
              ...
              <div style={s(`margin-top:5px; ${MONO}; font-size:10.5px; line-height:1.5; color:#8A8177`)}>rolling 7 days<br />target 90%</div>
            </div>
          </div>

          <div style={s(`padding:16px 18px; border-bottom:${LINE}`)}>
            <div style={s(`${MONO}; font-size:10.5px; color:#8A8177`)}>{d.loadLabel}</div>
            <div style={s('margin-top:16px; display:flex; align-items:flex-end; gap:5px; height:88px')}>
              {d.bars.map((h, i) => (
                <div key={i} data-bar={h} style={s(`flex:1; height:${h}%; border-radius:3px 3px 1px 1px; background:${h === 100 ? '#F4601E' : 'var(--dark-rule)'}`)} />
              ))}
            </div>
            <div style={s(`margin-top:10px; display:flex; justify-content:space-between; ${MONO}; font-size:10px; color:#8A8177`)}>
          ...
          <div style={s('padding:16px 18px; display:flex; flex-direction:column; gap:13px')}>
            <div style={s(`${MONO}; font-size:10.5px; color:#8A8177`)}>{d.queueLabel}</div>
            {d.queue.map((q, i) => (
              <div key={q.name}>
                <div style={s('display:flex; justify-content:space-between; align-items:baseline; font-size:12.5px')}>
                  <span>{q.name}</span>
                  <span style={s(`${MONO}; font-size:11px; color:#8A8177`)}>{q.n}</span>
                </div>
                <div style={s('margin-top:7px; height:4px; border-radius:99px; background:var(--dark-rule); overflow:hidden')}>
                  <div data-fill={q.pct} style={s(`height:100%; width:${q.pct}; border-radius:99px; background:${i === 0 ? '#F4601E' : '#6E6862'}`)} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={s(`margin-top:auto; border-top:${LINE}; padding:16px 18px`)}>
            <div style={s(`${MONO}; font-size:10.5px; color:#8A8177`)}>next scheduled report</div>
