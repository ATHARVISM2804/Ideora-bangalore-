  ...
  return (
    <section id="process" style={s('padding:0 0 clamp(36px, 5vw, 64px); position:relative')}>
      <div style={s('max-width:1400px; margin:0 auto; padding:0 clamp(20px, 5vw, 40px); text-align:center')}>
        ...
        <h2 data-anim="head" style={s('margin:28px auto 0; max-width:18ch; font-family:Archivo, sans-serif; font-stretch:125%; font-weight:500; font-size:clamp(28px, 5.2vw, 44px); line-height:0.94; letter-spacing:-0.035em')}>Easy process, powerful results</h2>
      ...
      <div style={s('position:relative; max-width:1400px; margin:clamp(44px, 7vw, 72px) auto 0; padding:0 clamp(20px, 5vw, 40px)')}>
        <div style={s(`position:absolute; top:0; bottom:0; left:${stacked ? 'clamp(20px, 5vw, 40px)' : '50%'}; width:1px; background:#D9DDE4`)} />
            ...
            <div key={p.step} className="om-g2" style={s('display:grid; grid-template-columns:1fr 1fr; align-items:center; column-gap:0')}>
              <div style={s(`grid-column:${p.cardCol}; grid-row:${stacked ? 2 : 1}; padding:${stacked ? '0 0 0 26px' : p.cardPad}`)}>
                ...
                >
                  ...
                  <div style={s('position:relative; height:220px; border-radius:14px; border:1px solid rgba(26,29,35,0.09); background:#F7F8FA; overflow:hidden')}>
      ...
      <div style={s('max-width:1400px; margin:clamp(48px, 7vw, 76px) auto 0; padding:0 clamp(20px, 5vw, 40px)')}>
        <div className="om-g4" style={s('border-radius:20px; border:1px solid rgba(26,29,35,0.07); background:rgba(255,255,255,0.85); box-shadow:0 30px 66px -44px rgba(26,29,35,0.55); overflow:hidden; display:grid; grid-template-columns:repeat(4,1fr)')}>
          {RESULT_STATS.map((r) => (
            <div key={r.label} data-anim="card" style={s('padding:28px 26px; border-right:1px solid rgba(26,29,35,0.07)')}>
              <div style={s('display:flex; align-items:baseline; gap:2px')}>
                <span data-count={r.value} style={s('font-family:Archivo, sans-serif; font-stretch:125%; font-weight:500; font-size:clamp(27px, 4.8vw, 40px); line-height:1; letter-spacing:-0.035em; color:#1A1D23')}>{r.value}</span>
