  ...
  return (
    <section id="book" style={s('padding:0 0 clamp(80px, 12vw, 170px)')}>
      ...
      <div style={s('max-width:var(--wide); margin:0 auto; padding:0 var(--gut)')}>
        <div onMouseMove={spot} style={s('position:relative; overflow:hidden; border-radius:16px; background:var(--raised); color:var(--ink); border:1px solid var(--rule); padding:clamp(48px, 6vw, 78px) clamp(28px, 5vw, 72px); box-shadow:0 40px 90px -60px rgba(28,25,23,0.45)')}>
          <div style={s('position:absolute; inset:0; pointer-events:none; background:radial-gradient(760px circle at var(--mx, 30%) var(--my, 50%), rgba(244,96,30,0.07), transparent 62%)')} />
          <div style={s('position:absolute; inset:0; pointer-events:none; background-image:radial-gradient(rgba(28,25,23,0.07) 1px, transparent 1px); background-size:26px 26px; mask-image:radial-gradient(circle at 78% 40%, #000, transparent 68%); -webkit-mask-image:radial-gradient(circle at 78% 40%, #000, transparent 68%)')} />
          <div className="om-g12" style={s('position:relative; display:grid; grid-template-columns:repeat(12, 1fr); gap:20px; align-items:center')}>
            <h2 data-anim="head" style={s('grid-column:1 / span 6; margin:0; font-family:var(--display); font-weight:600; font-size:clamp(31px, 5.2vw, 50px); line-height:1.1; letter-spacing:-0.016em')}>Bring us the process nobody wants to own.</h2>
