...
export function Work({ pinRef, trackRef, railRef }) {
  ...
  return (
    <section id="work" ref={pinRef} data-nav-dark style={s('position:relative; overflow:hidden; background:var(--dark); color:var(--bg); padding:clamp(64px, 9vw, 144px) 0')}>
      ...
      <div className="om-g12" style={s('position:relative; max-width:1400px; margin:0 auto; padding:0 clamp(20px, 5vw, 40px); display:grid; grid-template-columns:repeat(12,1fr); gap:20px; align-items:end')}>
        <h2 style={s('grid-column:1 / span 6; margin:0; font-family:var(--serif); font-weight:500; font-size:clamp(28px, 5.2vw, 44px); line-height:0.98; letter-spacing:-0.03em')}>Systems in production</h2>
        <p style={s('grid-column:8 / span 4; margin:0; color:#A79E93')}>Four systems running inside client operations, and two more in build. Named clients are under NDA, so each is described by what it does and what it replaced.</p>
      </div>
        ...
        <div ref={trackRef} style={s('display:flex; align-items:stretch; width:max-content')}>
          {CASES.map((cs, i) => (
            ...
            >
              ...
              <div style={s(`position:relative; display:flex; align-items:center; gap:12px; ${MONO}; font-size:12px; color:#8A8177`)}>
              ...
              <div style={s(`position:relative; margin-top:30px; padding-top:18px; border-top:1px solid var(--dark-rule); display:flex; flex-direction:column; gap:10px; ${MONO}; font-size:12px`)}>
