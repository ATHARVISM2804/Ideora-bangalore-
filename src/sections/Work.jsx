...
export function Work({ pinRef, trackRef, railRef }) {
  ...
  return (
    <section id="work" ref={pinRef} data-nav-dark style={s('position:relative; overflow:hidden; background:#1A1D23; color:#F1F3F6; padding:clamp(64px, 9vw, 144px) 0')}>
      ...
      <div className="om-g12" style={s('position:relative; max-width:1400px; margin:0 auto; padding:0 clamp(20px, 5vw, 40px); display:grid; grid-template-columns:repeat(12,1fr); gap:20px; align-items:end')}>
        <h2 style={s('grid-column:1 / span 6; margin:0; font-family:Archivo, sans-serif; font-stretch:125%; font-weight:500; font-size:clamp(28px, 5.2vw, 44px); line-height:0.98; letter-spacing:-0.03em')}>Systems in production</h2>
        <p style={s('grid-column:8 / span 4; margin:0; color:#A7AEBA')}>Four systems running inside client operations, and two more in build. Named clients are under NDA, so each is described by what it does and what it replaced.</p>
      </div>
      ...
      >
        <div ref={trackRef} style={s('display:flex; align-items:stretch; width:max-content')}>
          {CASES.map((cs, i) => (
            <div
              key={cs.code}
              style={s(`position:relative; flex:0 0 ${swipe ? 'min(82vw, 470px)' : '470px'}; padding:0 clamp(24px, 4vw, 44px) 0 ${i ? 'clamp(24px, 4vw, 44px)' : '0'}; ${i ? 'border-left:1px solid #2E333C' : ''}; ${swipe ? 'scroll-snap-align:start' : ''}`)}
            >
              {/* Ghost index sits behind the slide, giving the track a sense of
                  position without adding another boxed card. */}
              <span style={s('position:absolute; top:-14px; right:36px; font-family:Archivo, sans-serif; font-stretch:125%; font-weight:600; font-size:clamp(44px, 11vw, 112px); line-height:1; letter-spacing:-0.05em; color:rgba(255,255,255,0.035); pointer-events:none; user-select:none')}>{String(i + 1).padStart(2, '0')}</span>

              <div style={s(`position:relative; display:flex; align-items:center; gap:12px; ${MONO}; font-size:12px; color:#8A929E`)}>
                <span style={s(`color:${cs.status === 'live' ? '#FF8A50' : '#C6CCD6'}`)}>{cs.vertical}</span>
                {cs.status !== 'live' && (
                  <span style={s('padding:2px 8px; border-radius:99px; border:1px solid rgba(198,204,214,0.28); background:rgba(198,204,214,0.08); font-size:10px; color:#C6CCD6')}>{cs.status}</span>
                )}
                <span style={s('flex:1; height:1px; background:#2E333C')} />
                <span>{cs.code}</span>
              </div>

              <h3 style={s('position:relative; margin:26px 0 0; font-family:Archivo, sans-serif; font-stretch:125%; font-weight:500; font-size:clamp(21px, 3.5vw, 28px); line-height:1.06; letter-spacing:-0.03em')}>{cs.title}</h3>
              <p style={s('position:relative; margin:18px 0 0; max-width:40ch; font-size:15px; color:#A7AEBA')}>{cs.body}</p>

              <div style={s(`position:relative; margin-top:30px; padding-top:18px; border-top:1px solid #2E333C; display:flex; flex-direction:column; gap:10px; ${MONO}; font-size:12px`)}>
                ...
                <div style={s('display:flex; justify-content:space-between')}>
                  <span style={s('color:#8A929E')}>{SINCE_LABEL[cs.status]}</span>
                  <span style={s(`color:${cs.status === 'live' ? '#FF8A50' : '#C6CCD6'}`)}>{cs.since}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div style={s('position:relative; max-width:1400px; margin:44px auto 0; padding:0 clamp(20px, 5vw, 40px); display:flex; align-items:center; gap:14px')}>
        <span style={s(`${MONO}; font-size:11px; color:#8A929E`)}>{swipe ? 'swipe to advance' : 'scroll to advance'}</span>
