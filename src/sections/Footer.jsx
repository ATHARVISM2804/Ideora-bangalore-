...
export function Footer() {
  return (
    <footer style={s('border-top:1px solid var(--rule); background:rgba(255,255,255,0.62); backdrop-filter:blur(16px); padding:clamp(44px, 6vw, 80px) 0 36px; color:var(--ink-muted)')}>
      <div className="om-gfoot" style={s('max-width:var(--wide); margin:0 auto; padding:0 var(--gut); display:grid; grid-template-columns:repeat(12, 1fr); gap:20px')}>
      ...
      <div style={s('max-width:var(--wide); margin:clamp(32px, 5vw, 52px) auto 0; padding:24px clamp(20px, 5vw, 40px) 0; gap:12px; flex-wrap:wrap; border-top:1px solid var(--rule); display:flex; justify-content:space-between; font-size:13px')}>
